...
public sealed partial class StrategyEditor
{
    ...
    private static double SliderNumber(string value, double fallback) =>
        double.TryParse(value, NumberStyles.Float, CultureInfo.InvariantCulture, out var parsed)
            && !double.IsNaN(parsed) && !double.IsInfinity(parsed) ? parsed : fallback;
    ...
    internal void RebuildWeightEditor(JsonObject question, JsonObject strategy, string normalizedType)
    {
        SyncingWeights = true;
        WeightOptions.Clear();
        WeightLabels.Clear();
    ...
    internal void ApplyBiasPreset(string bias)
    {
        if (SliderValue)
        {
            return;
    ...
    internal void UpdateRatioPreview()
    {
