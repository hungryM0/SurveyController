...
public sealed partial class StrategyEditor
{
    ...
    internal void ApplyQuestionTypeBrush(string type)
    {
    ...
    internal void LoadAdvancedEditors(JsonObject question, JsonObject strategy, WizardQuestion summary)
    {
        ...
        foreach (var value in fillable)
        {
            ...
            ControlHelper.SetHeader(text, "填写文本");
            ...
            var panel = new SimpleStackPanel { Spacing = 8 };
        ...
        for (var index = 0; index < count; index++)
        {
            ...
            var label = index < labels.Count && AsString(labels[index]).Length > 0
                ...
                : $"第 {index + 1} 个填空";
            var panel = new SimpleStackPanel { Spacing = 8 };
            panel.Children.Add(new TextBlock { FontWeight = FontWeights.SemiBold, Text = label });
            panel.Children.Add(mode);
            panel.Children.Add(range);
            panel.Children.Add(ai);
        ...
        foreach (var value in attached)
        {
            ...
            for (var index = 0; index < selectTexts.Count; index++)
            {
                ...
                var slider = new Slider { Minimum = 0, Maximum = 100, SmallChange = 1, LargeChange = 1, Value = weight, Margin = new Thickness(8, 0, 8, 0) };
    ...
    internal void SaveAdvancedEditors(JsonObject question, string normalizedType, JsonObject changes)
    {
    ...
    private static DecodedFill DecodeFillValue(string value)
    {
        ...
        {
            var payload = value.Substring(RandomIntegerPrefix.Length);
            var separator = payload.IndexOf(':');
            if (separator > 0
                && double.TryParse(payload.Substring(0, separator), NumberStyles.Float, CultureInfo.InvariantCulture, out var minParsed)
                && double.TryParse(payload.Substring(separator + 1), NumberStyles.Float, CultureInfo.InvariantCulture, out var maxParsed))
            {
    ...
    private static Border CreateRowSurface(UIElement child)
    {
        ...
        return new Border
        {
            ...
            Background = (resources["CardBackgroundFillColorDefaultBrush"] ?? resources["SystemControlBackgroundAltHighBrush"]) as Brush,
            BorderBrush = (resources["CardStrokeColorDefaultBrush"] ?? resources["SystemControlForegroundBaseLowBrush"]) as Brush,
            Child = child,
        };
    }

    private static SimpleStackPanel CreateRangePanel(NumberBox minimum, NumberBox maximum)
    {
        ...
        minimum.Width = 150;
        maximum.Width = 150;
        return new SimpleStackPanel { Orientation = Orientation.Horizontal, Spacing = 8, Children = { minimum, maximum } };
    }
