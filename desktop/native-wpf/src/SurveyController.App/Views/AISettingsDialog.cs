...
internal static class AISettingsDialog
{
    public static async Task<bool> ShowAsync()
    {
        ...
        var panel = new SimpleStackPanel { Spacing = 12 };
        ...
        var baseUrl = new TextBox { Text = Str(profile, "baseURL", string.Empty) };
        var baseUrlPanel = new SimpleStackPanel { Spacing = 4 };
        baseUrlPanel.Children.Add(new TextBlock { Text = "接口地址" });
        baseUrlPanel.Children.Add(baseUrl);

        var model = new TextBox { Text = Str(profile, "model", string.Empty) };
        var modelPanel = new SimpleStackPanel { Spacing = 4 };
        modelPanel.Children.Add(new TextBlock { Text = "模型" });
        modelPanel.Children.Add(model);

        var hasSavedKey = Bool(profile, "hasAPIKey");
        var apiKey = new PasswordBox();
        var apiKeyPanel = new SimpleStackPanel { Spacing = 4 };
        apiKeyPanel.Children.Add(new TextBlock { Text = hasSavedKey ? "API 密钥 (已安全保存，留空不修改)" : "API 密钥" });
        apiKeyPanel.Children.Add(apiKey);

        var testLabel = new TextBlock { Text = "测试连接" };
        var testContent = new SimpleStackPanel { Orientation = Orientation.Horizontal, Spacing = 8 };
        ...
        panel.Children.Add(modelPanel);
        panel.Children.Add(apiKeyPanel);
        ...
        void UpdateVisibility()
        {
            ...
            baseUrlPanel.Visibility = customMode && SelectedTag(provider, "deepseek") == "custom"
                ? Visibility.Visible
                : Visibility.Collapsed;
            modelPanel.Visibility = customMode ? Visibility.Visible : Visibility.Collapsed;
            apiKeyPanel.Visibility = customMode ? Visibility.Visible : Visibility.Collapsed;
