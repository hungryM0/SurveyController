...
public sealed partial class StrategyEditor
{
    private void RebuildQuestionTree(int selectedIndex)
    {
        ...
        for (var index = 0; index < questions.Count; index++)
        {
            ...
            if (!questionIndices.ContainsKey(questions[index].Number))
            {
                questionIndices[questions[index].Number] = index;
            }
        }

        var pages = new Dictionary<int, TreeViewItem>();
        TreeViewItem PageNode(int page)
        {
            ...
            QuestionTree.Items.Add(node);
            pages[page] = node;
            return node;
        }

        void AppendRelation(TreeViewItem parent, string label, int targetIndex)
        {
            var node = new TreeViewItem { Header = label };
        ...
        for (var index = 0; index < questions.Count; index++)
        {
            ...
            var node = new TreeViewItem { Header = BuildQuestionNodeContent(summary) };
            parent.Items.Add(node);
            ...
            if (!hasUnknownLogic)
            {
                foreach (var value in JsonArrayField(question, "controls_display_targets"))
                {
                    ...
                    var targetIndex = questionIndices.TryGetValue(targetNumber, out var ti) ? ti : index;
            ...
            node.IsExpanded = node.Items.Count > 0;
        }

        if (QuestionTree.Items.Count == 0)
        {
            _questionIndex = -1;
    ...
    private void SetTreeSelection(int index)
    {
        ...
        if (!ReferenceEquals(QuestionTree.SelectedItem, node))
        {
    ...
    private void OnQuestionSelected(object sender, RoutedPropertyChangedEventArgs<object> e)
    {
    ...
    internal static string ShortTitle(string value)
    {
        ...
        const int limit = 24;
        return title.Length > limit ? title.Substring(0, limit - 1) + "…" : title;
    }
    ...
    private static SimpleStackPanel BuildQuestionNodeContent(WizardQuestion question)
    {
        ...
        var row = new SimpleStackPanel { Orientation = Orientation.Horizontal, Spacing = 7 };
