...
public sealed class ProcessBackendTransport : BackendTransport
{
    ...
    public override void Kill()
    {
        try
        {
            if (!_process.HasExited)
            {
                ...
                // Unix 上的终止是异步的；短暂等待让 HasExited 反映真实状态。
    ...
    public static string BackendPath()
    {
        ...
        return Path.Combine(Path.GetDirectoryName(exePath) ?? ".", "SurveyController.Backend.exe");
    }
    ...
    private static async Task BeginDrain(Process process)
    {
        try
        {
            await process.StandardError.ReadToEndAsync().ConfigureAwait(ConfigureAwaitOptions.None);
        }
        catch (Exception)
        {
