...
public sealed partial class RuleEditor : UserControl
{
    ...
    public void Refresh()
    {
        ...
        try
        {
            ...
            {
                _ruleIndex = Math.Clamp(_ruleIndex < 0 ? 0 : _ruleIndex, 0, rules.Count - 1);
    ...
    private void LoadQuestionPicker(AutoSuggestBox box, bool target)
    {
        ...
        foreach (var question in _document.Questions())
        {
            ...
            if (needle.Length == 0 || label.ToLowerInvariant().Contains(needle, StringComparison.Ordinal))
            {
    ...
    private static int ParseQuestionNumber(string label)
    {
        ...
        return int.TryParse(label[start..end], NumberStyles.Integer, CultureInfo.InvariantCulture, out var number)
            ? number
            : 0;
    }
