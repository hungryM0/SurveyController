...
public sealed partial class TaskPage
{
    ...
    private async Task<string> ChooseSaveFileAsync(bool json)
    {
        var picker = new FileSavePicker(WindowContext.MainWindowId)
        {
            SuggestedFileName = json ? "surveycontroller-result" : "surveycontroller-runtime",
            DefaultFileExtension = json ? ".json" : ".log",
        };
        picker.FileTypeChoices.Add(json ? "JSON 文件" : "日志文件", new List<string> { json ? ".json" : ".log" });
