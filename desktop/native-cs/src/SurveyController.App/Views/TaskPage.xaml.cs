...
/// </summary>
public sealed partial class TaskPage : Page, IShutdownAware
{
    ...
    private bool SyncControlsToDocument()
    {
        ...
        alphaValue = Math.Clamp(alphaValue, 0.5, 0.99);
    ...
    private void OnEditAnswers(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            window.ShowForOwner(WindowContext.MainWindowId);
        }
        catch (Exception error)
        {
            _answerEditor = null;
    ...
    private void OnSurveyUrlChanged(object sender, RoutedEventArgs e)
    {
    ...
    private async Task<string> ChooseFileAsync(bool image, bool spreadsheet = false)
    {
        var picker = new FileOpenPicker(WindowContext.MainWindowId);
        if (image)
        {
    ...
    private static bool ParseWindowValue(string value, CalendarDatePicker datePicker, TimePicker timePicker)
    {
        if (value.Length == 0)
        {
        ...
        datePicker.Date = new DateTimeOffset(parsed);
        timePicker.SelectedTime = parsed.TimeOfDay;
        return true;
    }

    private static bool ReadWindowValue(CalendarDatePicker datePicker, TimePicker timePicker,
        out string value, out string error)
    {
        ...
        var local = date.Value.LocalDateTime.Date;
        var combined = local.Add(time.Value);
