"""UI 辅助工具函数。"""

from software.ui.helpers.contact_api import format_quota_value as contact_format_quota_value
from software.ui.helpers.contact_api import get_session_snapshot as get_contact_session_snapshot
from software.ui.helpers.contact_api import post as post_contact_request
from software.ui.helpers.proxy_access import (
    PROXY_SOURCE_BENEFIT,
    RandomIPAuthError,
    apply_custom_proxy_api,
    apply_proxy_area_code,
    apply_proxy_source_settings,
    claim_easter_egg_bonus,
    format_quota_value,
    format_random_ip_error,
    format_status_payload,
    get_proxy_minute_by_answer_seconds,
    get_proxy_settings,
    get_quota_cost_by_minute,
    get_random_ip_counter_snapshot_local,
    get_session_snapshot,
    has_authenticated_session,
    has_unknown_local_quota,
    is_quota_exhausted,
    load_area_codes,
    load_benefit_supported_areas,
    load_supported_area_codes,
    test_custom_proxy_api,
)
from software.ui.helpers.qfluent_compat import install_qfluentwidgets_animation_guards

__all__ = [
    "PROXY_SOURCE_BENEFIT",
    "RandomIPAuthError",
    "apply_custom_proxy_api",
    "apply_proxy_area_code",
    "apply_proxy_source_settings",
    "claim_easter_egg_bonus",
    "contact_format_quota_value",
    "format_quota_value",
    "format_random_ip_error",
    "format_status_payload",
    "get_contact_session_snapshot",
    "get_proxy_minute_by_answer_seconds",
    "get_proxy_settings",
    "get_quota_cost_by_minute",
    "get_random_ip_counter_snapshot_local",
    "get_session_snapshot",
    "has_authenticated_session",
    "has_unknown_local_quota",
    "install_qfluentwidgets_animation_guards",
    "is_quota_exhausted",
    "load_area_codes",
    "load_benefit_supported_areas",
    "load_supported_area_codes",
    "post_contact_request",
    "test_custom_proxy_api",
]
