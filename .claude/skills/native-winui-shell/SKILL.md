...
1. 先读根目录 `DESIGN.md`、目标页面、相邻 `.xaml.cpp/.h` 和相关 `Services/`。界面契约以 `DESIGN.md` 和当前资源字典为准。
...
5. `RpcServices` 提供业务调用，`BackendClient` 只负责匿名管道传输和后端进程生命周期。新增能力同步 Go 服务、RPC 分派、`RpcServices` 和页面错误反馈；禁止伪造成功状态。
