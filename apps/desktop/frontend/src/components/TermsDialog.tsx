...
【信息收集】
  ...
  · 软件会创建并在本机保存设备标识，并从官方服务同步用户标识与额度信息
  · 提交结果遥测默认开启。任务结束后会向官方服务发送用户标识、设备标识、
    完整问卷链接、提交结果、代理来源和客户端版本，用于身份、额度与提交统计
  · 提交结果遥测不包含问卷答案内容和 AI API 密钥，可在设置中关闭
  · 问卷配置和 AI API 密钥保存在本地配置与设置文件中
...
【第三方服务】
  ...
  · 官方身份、额度与提交统计服务（应用运行和提交结果遥测会调用）
  · AI 服务提供商（若用户主动配置并启用 AI 功能）
  · 随机 IP 代理服务商（任务配置或官方代理功能调用时）
...
【数据安全】
  ...
  · 用户配置的 API 密钥仅存储于本地配置与设置文件，不随提交结果遥测上传
  · 软件运行日志记录任务状态和技术诊断信息，用户应妥善保管日志与配置文件
  · 用户可删除本地配置文件和设置数据，清除本机保存的信息`,
}
