...
def _parse_session_response(response: Any) -> RandomIPSession:
    ...
    session = RandomIPSession(
        ...
        total_quota=_to_non_negative_int(data.get("remaining_quota"), 0),
    )
...
def refresh_session(*, force: bool = False) -> RandomIPSession:
    ...
    refreshed = RandomIPSession(
        ...
        total_quota=max(_to_non_negative_int(data.get("remaining_quota"), session.remaining_quota), session.total_quota),
    )
...
def extract_proxy(*, minute: int, pool: str, area: Optional[str]) -> Dict[str, Any]:
    ...
    for attempt in range(2):
        ...
        if status_code == 200:
            ...
            update_remaining_quota(remaining_quota, total_hint=remaining_quota + quota_cost)
            return {
                "host": host,
                "port": port,
